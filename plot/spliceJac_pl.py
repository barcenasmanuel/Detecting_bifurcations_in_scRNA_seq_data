...
def plot_stability(adata, filename_p):
    ...
    smooth_tm, smooth_avg = tools.smooth_curve(tm, avg_max, ints, rep=1)
    smooth_tm, smooth_pos = tools.smooth_curve(tm, avg_pos, ints, rep=1)

    matplotlib.rc('xtick', labelsize=small_size)
    matplotlib.rc('ytick', labelsize=small_size)
    fig = plt.figure(figsize=(3.54, 10.62))  # figsize=(WIDTH_SIZE,HEIGHT_SIZE); figsize = (3.54,3.54)
    # fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(3.54, 3.54))

    ax1 = plt.subplot(311)
    plt.errorbar(tm, avg_pos, yerr=std_pos, fmt='o')
    plt.plot(smooth_tm, smooth_pos, 'r--')
    plt.ylabel('Positive eigenvalues', fontsize=medium_size)
    ...
    plt.ylabel('Largest eigenvalue', fontsize=medium_size)
    plt.xlabel('Pseudotime', fontsize=medium_size)
    # plt.xticks([])
    ...
    plt.xlabel('Pseudotime', fontsize=medium_size)
    plt.ylabel('Number of observables', fontsize=medium_size)

    plt.tight_layout()

    # plt.savefig(f'figures/{filename_p}/regression_{filename_p}.pdf', format='pdf',bbox_inches='tight', dpi = 300)
    plt.savefig(f'figures/{filename_p}/regression_{filename_p}.png', format='png', bbox_inches='tight', dpi=300)
    # plt.savefig('figures/regression_OVCA420_TGFB1.pdf', format='pdf')
    # plt.savefig('figures/regression_EGF.pdf', format='pdf')
    # plt.savefig('figures/regression_MCF7_TGFB1.pdf', format='pdf')
